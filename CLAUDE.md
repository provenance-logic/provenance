...
> **In progress (2026-04-30):** Phase 5 (Open Source Ready) continues. **Domain 11 (Notifications) is fully shipped** — all 12 trigger-bundle PRs merged plus PR #6 (notification center frontend, merged as #42) and PR #9b (F11.17 trust-score-significant-change). All 27 PRD trigger requirements wired or explicitly deferred (remaining deferrals: F11.14 subscription model, F11.15 schema-drift detection, F11.19 re-eval-on-publish, F11.22 classification post-publish mutability, F11.24/25/26 auto-suspension / human review queue / frozen-state machine). Frontend post-merge fixes also shipped: TypeORM entity registration in the global data source, deep-link resolver for stale trigger URLs, catch-all NotFound route. **Domain 10 Workstream B** mostly shipped (F10.5, F10.6, F10.7 partial, F10.8, F10.9, F10.10). Remaining additive: per-driver SQL probes, per-storage file probes, schema authoring F10.11–F10.13. **Domain 12 (Connection References and Per-Use-Case Consent)** — partial. Shipped: data primitives (V18/V19), state machine (`ConsentService` — request, approve, deny, principal-revoke, grant-revoke cascade), REST surface at `/organizations/:orgId/consent/connection-references`, connection package emission at activation per ADR-008, F12.10 notification fan-out (in Domain 11 PR #12). Remaining: runtime scope enforcement at Agent Query Layer (F12.16+, ADR-006), automatic expiration (F12.22), MAJOR-version suspension (F12.15), governance override (F12.14, F12.20), legacy-agent migration (F12.25), Supervised oversight-hold sub-state, rest of F12.21 cascade triggers (product lifecycle, owner deactivation), per-reference scope filtering on package (ADR-008 scope inheritance). **Phase 5.5 / 5.6 / 5.7** remain. **Domain 9 walkthrough gaps from the April 18 review — all six closed tonight** across five PRs (cross-org Request Access guard merged #43, test-artifact cleanup merged #44; P1 enrichment rendering, lifecycle visibility, real port contract schemas, README refresh, and this v1.5 cleanup all open). See `documents/prd/implementation-status.md` for the authoritative per-feature status and the 10-item OSR blocker list.
