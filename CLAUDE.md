...
> **In progress (2026-04-26):** Phase 5 (Open Source Ready) continues. **Domain 10 Workstream B** — F10.5, F10.6 (end-to-end verified 2026-04-25), F10.8, F10.9, F10.10 (refresh on connection-detail edit, end-to-end verified 2026-04-25), and F10.7 (REST + GraphQL + Kafka real probes with frontend `ProbeStatusBadge`; SQL/JDBC and file_object_export return a typed `unsupported` response) implemented and deployed. Remaining: per-driver SQL probes, per-storage file probes (additive). **Domain 11 (Notifications)** — ADR-009 accepted; **all 12 trigger-bundle PRs shipped** (#2/#3/#5/#4/#7/#8/#9/#10/#11/#12) plus PR #6 (notification center frontend) **in flight on `feat/notifications-frontend` — code complete, typechecks pass, awaiting PR open + browser smoke-test before merge**. V21–V26 schemas. Implemented: F11.1–F11.3, F11.4 (frontend in PR #6), F11.5–F11.13, F11.16, F11.18, F11.20, F11.21, F11.23, F11.27. Also F12.10. Deferrals (each documented in `documents/prd/implementation-status.md`): F11.14 (no subscription model), F11.15 (no schema-drift detection), F11.17 (PR #9b — trust-score-significant-change), F11.19 (no re-eval-on-publish), F11.22 (classification frozen post-publish), F11.24/25/26 (no auto-suspension / human review queue / frozen-state machine yet).

> **Session resume note (2026-04-27):** PR #6 frontend is committed on the branch `feat/notifications-frontend` and pushed to origin but the GitHub PR has not been opened yet. To resume: open the PR at https://github.com/provenance-logic/provenance/pull/new/feat/notifications-frontend, smoke-test the bell + drawer + preferences in the browser, then merge. After that the only Domain 11 work remaining is **PR #9b (F11.17 trust-score-significant-change)** plus the per-trigger deferrals listed above. **Domain 12 (Connection References and Per-Use-Case Consent)** — data primitives, state machine, REST surface, and package emission at activation shipped (F12.1–F12.6 schema; F12.9 request; F12.12 deny; F12.13 approve with ADR-008 package emission; F12.19 principal-initiated revocation; F12.21 grant-revoke cascade; REST + OpenAPI at `/organizations/{orgId}/consent/connection-references`). Remaining: Domain 11 notification fan-out (F12.10), Supervised oversight-hold sub-state (F12.9), governance override (F12.14, F12.20), MAJOR-version suspension (F12.15), runtime scope enforcement at the Agent Query Layer (F12.16+, ADR-006), automatic expiration (F12.22), legacy-agent migration (F12.25), product-lifecycle and owner-deactivation cascade triggers (rest of F12.21), per-reference scope filtering on the package (ADR-008 scope inheritance). Bug fixed today: R-011 — added `updated_at` column to `access.access_grants` (V20) so the existing trigger no longer blocks revocation, which would have blocked F12.21 cascade in production. **Phase 5.5 / 5.6 / 5.7** remain. See `documents/prd/implementation-status.md` for the full gap analysis, including the 10-item Open Source Readiness blocker list.
...
**Domain 12 (Connection References and Per-Use-Case Consent)** is cross-cutting work rather than a build phase. Its requirements (F12.1–F12.25, NF12.1–NF12.8, PRD v1.5) and architecture (Architecture v1.5 Section 3, ADR-005 through ADR-008) are complete; it is implemented within Phase 5 as OSR blocker #10, sequenced after Domain 11 Notifications and Domain 10 Workstream B complete. **Partial as of 2026-04-24** — data layer, service-layer state machine, REST surface, and package-at-activation have landed; runtime enforcement at the Agent Query Layer and notification-dependent features remain.

**Active phase: 5 (Open Source Ready).** Phases 1–4 complete as of April 13, 2026. Phase 5 progress as of April 24, 2026 (see `documents/prd/implementation-status.md` for the authoritative 10-item Open Source Readiness blocker list, which now includes Domain 12 Connection References as blocker #10):
...
- 🔄 **Domain 12 — Connection References and Per-Use-Case Consent** — partial as of 2026-04-24. Shipped: V18/V19 schema (`consent.connection_references` + outbox + connection package column), `ConsentService` state machine (request / approve / deny / principal-revoke / grant-revoke cascade), REST controller at `/organizations/{orgId}/consent/connection-references` with OpenAPI spec, connection package emission at activation (ADR-008). Remaining: Domain 11 notification fan-out (F12.10), Supervised oversight-hold sub-state, governance override (F12.14, F12.20), MAJOR-version suspension (F12.15), runtime scope enforcement at Agent Query Layer (F12.16+, ADR-006), automatic expiration (F12.22), legacy-agent migration (F12.25), rest of the F12.21 cascade triggers (product lifecycle, owner deactivation), per-reference scope filtering on the package (ADR-008 scope inheritance).
