...
> **In progress (2026-04-23):** Phase 5 (Open Source Ready) continues. **Domain 10 Workstream B** — F10.5, F10.8, F10.9 implemented and deployed; F10.6 (confidentiality + access-gated disclosure) wired end to end but UI disclosure flow not yet verified; F10.7 (automated connectivity validation) stubbed as 501; F10.10 (package refresh) not started; F10.11–F10.13 (schema authoring) untouched. **Domain 11 (Notifications)** — F11.1–F11.26 not implemented; OSR blocker #2 and a hard prerequisite for Domain 12 enforcement. **Domain 12 (Connection References and Per-Use-Case Consent)** — requirements (F12.1–F12.25, NF12.1–NF12.8) and architecture (ADR-005 through ADR-008) complete; implementation not yet started; depends on Domain 11 and Domain 10 Workstream B. **Phase 5.5 / 5.6 / 5.7** remain. See `documents/prd/implementation-status.md` for the full gap analysis, including the 10-item Open Source Readiness blocker list.
...
**Domain 12 (Connection References and Per-Use-Case Consent)** is cross-cutting work rather than a build phase. Its requirements (F12.1–F12.25, NF12.1–NF12.8, PRD v1.5) and architecture (Architecture v1.5 Section 3, ADR-005 through ADR-008) are complete; it is implemented within Phase 5 as OSR blocker #10, sequenced after Domain 11 Notifications and Domain 10 Workstream B complete. No implementation yet.

**Active phase: 5 (Open Source Ready).** Phases 1–4 complete as of April 13, 2026. Phase 5 progress as of April 23, 2026 (see `documents/prd/implementation-status.md` for the authoritative 10-item Open Source Readiness blocker list, which now includes Domain 12 Connection References as blocker #10):
...
- 🔲 **Domain 12 — Connection References and Per-Use-Case Consent** — requirements (F12.1–F12.25) and architecture (ADR-005 through ADR-008) complete; implementation not started. Depends on Domain 11 (Notifications, blocker #2) and Domain 10 Workstream B (blocker #8).
