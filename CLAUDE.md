...
> **In progress (2026-05-14):** Phase 5 (Open Source Ready) continues. **Domain 10 Workstream B** — F10.5, F10.8, F10.9 implemented and deployed; F10.6 (confidentiality + access-gated disclosure) wired end to end but UI disclosure flow not yet verified; F10.7 (automated connectivity validation) stubbed as 501; F10.10 (package refresh) not started; F10.11–F10.13 (schema authoring) untouched. **Domain 11 (Notifications)** — F11.1–F11.26 not implemented; OSR blocker #2 and the gating dependency for the remaining Domain 12 notification work. **Domain 12 (Connection References and Per-Use-Case Consent)** — data primitives, state machine, REST surface, package emission at activation, and runtime scope enforcement at the Agent Query Layer all shipped for product-targeted MCP tools (F12.1–F12.6 schema; F12.9 request; F12.12 deny; F12.13 approve with ADR-008 package emission; F12.16/F12.24 enforcement + denial audit via ADR-006 Phase 1, synchronous PG path; F12.19 principal-initiated revocation; F12.21 grant-revoke cascade; REST + OpenAPI at `/organizations/{orgId}/consent/connection-references`; internal lookup at `/internal/consent/connection-references/active`). Remaining: Domain 11 notification fan-out (F12.10), Supervised oversight-hold sub-state (F12.9), governance override (F12.14, F12.20), MAJOR-version suspension (F12.15), F12.17/F12.18 (trust-classification runtime differentiation, envelope verification), ADR-006 Phases 2–4 (in-memory cache + Redpanda invalidation + OPA hook), automatic expiration (F12.22), legacy-agent migration (F12.25), product-lifecycle and owner-deactivation cascade triggers (rest of F12.21), per-reference scope filtering on the package (ADR-008 scope inheritance). **Phase 5.5 / 5.6 / 5.7** remain. See `documents/prd/implementation-status.md` for the full gap analysis, including the 10-item Open Source Readiness blocker list.
