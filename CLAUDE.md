...
| connectors | connectors, connector_health_events, source_registrations, schema_snapshots, **capability_manifests, discovery_crawl_events, connector_secrets** | `connector_secrets` (V40) holds AES-256-GCM envelopes for self-service credential entry (ADR-013); `connectors.credential_arn` stores a `vault:<uuid>` reference to the encrypted row. Credentials are encrypted at rest — plaintext never in any column, log, or response. **No `discovery_coverage_scores` table today** — coverage levels live as labels inside `capability_manifests.capabilities_doc` JSONB; whether a per-crawl scoring table ships is part of the B-063 weekend overhaul. |
...
The demo environment is an on-demand clone provisioned per demo from git, with curated seed data. It is not a persistent staging tier — teardown is the lifecycle end-state (ADR-004). It typically spins up at T-24h before a demo and tears down after; while it's in active use (e.g. a self-paced walkthrough or a multi-day iteration window), it may be kept running between sessions rather than cycled down each time. To push merged `main` to a *standing* box, use the box's deploy script rather than a bare `git pull` + `up -d` (which does NOT reload app code — the stack builds images locally and api/web/agent-query bind-mount the source, so only a `--force-recreate` reloads them): **demo → `infrastructure/scripts/demo-sync.sh main`** (force-recreate + Caddy + integrity check + reseed); **dev → `infrastructure/scripts/refresh-ec2-dev.sh`** (force-recreate + Caddy + health poll, no reseed — dev holds ad-hoc state). `seed:reset:hard` is safe + repeatable on an already-seeded box as of 2026-05-30 (B-078 + B-085).
...
**Never persist plaintext credentials (ADR-013).** The platform may *receive* a connector credential in order to vault it, but must never persist the plaintext (no plaintext column, table, cache, or log line) and must never return it to any client. Credentials are encrypted at rest (AES-256-GCM via `EncryptionService`, the same key source as Domain 10 connection details: `CONNECTION_DETAILS_SECRET_ARN` or `CONNECTION_DETAILS_DEV_KEY`) and referenced by one of three schemes: `vault:<uuid>` (platform-managed encrypted store — the default for self-service GUI entry), `arn:aws:secretsmanager:…` (pre-staged AWS Secrets Manager), or `local-env:VARNAME` (dev/test only — never a user-facing path). `connectionConfig` must still never contain raw credentials — the `raw-credential-guard` `FORBIDDEN_KEYS` check is unchanged. The encrypted store is `connectors.connector_secrets`; every query on it must filter on `orgId`.
...
* Credentials are encrypted at rest (AES-256-GCM) and referenced by `vault:<uuid>` / `arn:` / `local-env:` — never plaintext in any column, log, cache, or response. The `connectors.connector_secrets` table holds only the AES-256-GCM envelope. Key source: `CONNECTION_DETAILS_SECRET_ARN` (prod) or `CONNECTION_DETAILS_DEV_KEY` (dev). See ADR-013.
...
* Discovery crawl credentials use the same vault/ARN/local-env pattern as connector credentials — plaintext never stored, encrypted at rest via `EncryptionService`
