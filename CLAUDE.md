...
> **In progress (2026-04-26):** Phase 5 (Open Source Ready) continues. **Domain 10 Workstream B** — F10.5, F10.6 (end-to-end verified 2026-04-25), F10.8, F10.9, F10.10 (refresh on connection-detail edit, end-to-end verified 2026-04-25), and F10.7 (REST + GraphQL + Kafka real probes with frontend `ProbeStatusBadge`; SQL/JDBC and file_object_export return a typed `unsupported` response) implemented and deployed. Remaining: per-driver SQL probes, per-storage file probes (additive). **Domain 11 (Notifications)** — ADR-009 accepted; in-platform tier (PR #2), email channel (PR #3), and per-principal preferences (PR #5) shipped. Phasing was reordered after PR #3: PR #5 (preferences) shipped before PR #4 (webhook) since webhook URL configuration belongs with webhook delivery. V21/V22/V23 schemas: notifications, delivery_outbox (RLS-free internal queue), principal_preferences. `NotificationsService.enqueue` consults per-(principal, category) preferences via `channel-resolver.ts` at enqueue time and writes outbox rows accordingly. `NotificationDeliveryWorker` drains every 30s via the existing platform-wide `EmailService`. F11.1 partial (no triggers wired yet), F11.2 partial (in-platform + email), F11.3 implemented, F11.4 partial (backend), F11.5 implemented. Webhook channel (PR #4), notification center UI (PR #6), and trigger-bundle wiring (PRs #7–12) remain. Still blocking Domain 12 fan-out (F12.10) until trigger bundles ship. **Domain 12 (Connection References and Per-Use-Case Consent)** — data primitives, state machine, REST surface, and package emission at activation shipped (F12.1–F12.6 schema; F12.9 request; F12.12 deny; F12.13 approve with ADR-008 package emission; F12.19 principal-initiated revocation; F12.21 grant-revoke cascade; REST + OpenAPI at `/organizations/{orgId}/consent/connection-references`). Remaining: Domain 11 notification fan-out (F12.10), Supervised oversight-hold sub-state (F12.9), governance override (F12.14, F12.20), MAJOR-version suspension (F12.15), runtime scope enforcement at the Agent Query Layer (F12.16+, ADR-006), automatic expiration (F12.22), legacy-agent migration (F12.25), product-lifecycle and owner-deactivation cascade triggers (rest of F12.21), per-reference scope filtering on the package (ADR-008 scope inheritance). Bug fixed today: R-011 — added `updated_at` column to `access.access_grants` (V20) so the existing trigger no longer blocks revocation, which would have blocked F12.21 cascade in production. **Phase 5.5 / 5.6 / 5.7** remain. See `documents/prd/implementation-status.md` for the full gap analysis, including the 10-item Open Source Readiness blocker list.
