...
> **In progress (2026-04-26):** Phase 5 (Open Source Ready) continues. **Domain 10 Workstream B** — F10.5, F10.6 (end-to-end verified 2026-04-25), F10.8, F10.9, F10.10 (refresh on connection-detail edit, end-to-end verified 2026-04-25), and F10.7 (REST + GraphQL + Kafka real probes with frontend `ProbeStatusBadge`; SQL/JDBC and file_object_export return a typed `unsupported` response) implemented and deployed. Remaining: per-driver SQL probes, per-storage file probes (additive). **Domain 11 (Notifications)** — ADR-009 accepted; PRs #2/#3/#5/#4 of 12 shipped (notification infra, email channel, preferences, webhook channel — phasing was reordered to land preferences before webhook so URL config landed with the webhook handler). V21/V22/V23/V24 schemas: notifications, delivery_outbox (RLS-free internal queue), principal_preferences, principal_settings (per-principal webhook URL). `NotificationsService.enqueue` consults per-(principal, category) preferences via `channel-resolver.ts` at enqueue time and writes outbox rows for in-platform / email / webhook accordingly. `NotificationDeliveryWorker` drains every 30s with FOR UPDATE SKIP LOCKED, NF11.3 retries; email via existing platform-wide `EmailService`, webhook via Node `fetch` with 10s `AbortController` timeout, https-only. F11.1 partial (no triggers wired yet), F11.2 implemented, F11.3 implemented, F11.4 partial (backend), F11.5 implemented. Notification center UI (PR #6), and trigger-bundle wiring (PRs #7–12) remain. Still blocking Domain 12 fan-out (F12.10) until trigger bundles ship. **Domain 12 (Connection References and Per-Use-Case Consent)** — data primitives, state machine, REST surface, and package emission at activation shipped (F12.1–F12.6 schema; F12.9 request; F12.12 deny; F12.13 approve with ADR-008 package emission; F12.19 principal-initiated revocation; F12.21 grant-revoke cascade; REST + OpenAPI at `/organizations/{orgId}/consent/connection-references`). Remaining: Domain 11 notification fan-out (F12.10), Supervised oversight-hold sub-state (F12.9), governance override (F12.14, F12.20), MAJOR-version suspension (F12.15), runtime scope enforcement at the Agent Query Layer (F12.16+, ADR-006), automatic expiration (F12.22), legacy-agent migration (F12.25), product-lifecycle and owner-deactivation cascade triggers (rest of F12.21), per-reference scope filtering on the package (ADR-008 scope inheritance). Bug fixed today: R-011 — added `updated_at` column to `access.access_grants` (V20) so the existing trigger no longer blocks revocation, which would have blocked F12.21 cascade in production. **Phase 5.5 / 5.6 / 5.7** remain. See `documents/prd/implementation-status.md` for the full gap analysis, including the 10-item Open Source Readiness blocker list.
