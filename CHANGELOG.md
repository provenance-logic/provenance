...
## [v0.1.0-osr] — 2026-05-24

**Open Source Ready — the first tagged release.** Provenance runs end to end as an agent-first, multi-tenant data-mesh coordination platform: org/domain onboarding, governed product publishing, marketplace discovery, lineage + trust score + SLOs, the nine-tool MCP agent layer with JWT auth, and per-use-case connection-reference consent with runtime scope enforcement. Four first-class connectors now work end to end — **PostgreSQL, Amazon S3, Databricks, and Snowflake** — closing B-063, the last OSR blocker.

Explicitly out of scope for this milestone (tracked, deferred): production hardening (Kubernetes, managed AWS, SOC 2 Type II — Phase 6), agent anomaly detection (5.5), SOC 2 foundations (5.7), and the B-075 Surface 2 publishing-front-door polish.

### Added

- **Snowflake first-class connector** ([#197](https://github.com/provenance-logic/provenance/pull/197)–[#202](https://github.com/provenance-logic/provenance/pull/202)). Inbound key-pair-JWT connectivity probe, `INFORMATION_SCHEMA` schema inference, discovery crawl, and `OBJECT_DEPENDENCIES` asset-level lineage; outbound consumer destination snippets; cross-org native Secure Data Sharing. Verified end to end against real Snowflake trial accounts (probe, schema, crawl, lineage into Postgres + Neo4j, and a real share mounted between two accounts). **Closes B-063** — the first-class connector set (PostgreSQL / S3 / Databricks / Snowflake) is now real, not advertised-only. `ACCESS_HISTORY` query-derived lineage (Layer 4b) is a deferred enrichment, not a blocker.
- **Connector detail page** ([#205](https://github.com/provenance-logic/provenance/pull/205)). New `/connectors/:connectorId` route that finally surfaces the discovery a registered connector actually ran: validation + re-crawl actions, a crawl-summary stat strip (tables found / sources created / snapshots / lineage edges) with a prior-crawl history accordion, and a discovered-sources table. Closes **B-075 Surface 1's** core complaint — a registered connector was previously a UI dead-end.
- **Create-product-from-source deep-link** ([#206](https://github.com/provenance-logic/provenance/pull/206)). The per-source "Create data product" button now threads the discovered source through product authoring into a pre-bound output port (it was a stub that dropped the source and navigated to `/dashboard`). **B-075 Surface 1 Tier A.** Auto-defaulting the contract schema from the discovered snapshot (so a bound port needs no hand-written JSON) is the named next slice, tracked under B-075 Surface 2.

> The 2026-05-15 → 05-24 consumer-grade-outbound arc (B-070 port↔source binding, the six `sql_jdbc` destination snippets, F10.14 catalog names, F10.15 Situation-A eligibility, F10.19 credential lifespan, F7.48 Account Surface) landed between this entry and the 2026-05-14 section below. Those PRs were not individually changelogged at the time; see `documents/status-board.md` and the git log for the per-PR detail.
...
- **Compose env validator (B-028 fix 3)** at `infrastructure/scripts/validate-compose-env.mjs` plus a `compose-env-allowlist.txt` companion. Scans `apps/api/src/**` and `apps/agent-query/src/**` for `process.env.X` references, `process.env['X']` brackets, AND zod-schema env-key declarations (the `KEY: z.string()...` pattern that hid `AQL_INTERNAL_TOKEN` from review the first time). Diffs against the `environment:` block of each service in `docker-compose.ec2-dev.yml`; fails the new `Validate Compose Env` CI job if any code-referenced var is undeclared and unallowlisted. **Closes B-028 in full** (fix 1 in #95, fix 2 in #96, fix 3 here). The validator caught three latent env-drift bugs on first run (`AQL_INTERNAL_TOKEN`, `KAFKA_BROKERS`, `CONNECTION_REFERENCE_ENFORCEMENT_ENABLED` all missing from the agent-query compose block) — fixed in the same PR. Open bug list is now zero Medium-severity items.
- **Sample-data button (B-027)** — new `apps/api/src/sample-data/` module with one endpoint `POST /organizations/:orgId/sample-data` gated by `RolesGuard` (`@Roles('org_admin')`) and a `DEMO_DATA_ENABLED` env flag (service throws 404 when off so production can't accidentally expose the surface). Idempotently populates the calling org with one domain, two products (one published, one draft), one output port per product with filled-in contract schemas, one freshness SLO, and two notifications. The OnboardingWizard's `confirm_org` step gains a `SampleDataAffordance` row with a "Populate sample data" button (confirm dialog before submit; replaces itself with a green summary on success; hides silently if the endpoint returns 404 so production users never see a broken feature). Closes **B-027**, the last F7.46 follow-on entry. Access-grant seeding deferred — meaningful grants require a second principal which expands scope; the wizard runs as org_admin who already has access to everything in their own org.
