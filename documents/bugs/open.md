...
## B-082 — MCP `get_product` returns HTTP 500 instead of a structured tool error when `domain_id` is omitted; downstream `"undefined"` reaches a TypeORM UUID column

- **Severity:** **High** — breaks the headline agent-discovery beat. `get_product` is the canonical "tell me about this data product" tool and the most natural product-bound call an agent makes after `list_products`. A 500 (rather than a structured MCP error) is also a misleading signal — operators chasing it will look at the connection-reference guard first when the guard actually *passes* before this fires.
- **Status:** Open. Discovered 2026-05-30 during end-to-end verification of the [PR #226 seed extension](https://github.com/provenance-logic/provenance/pull/226). The connection-reference guard (Domain 12 — active grant + active ref + scope match) succeeded and wrote the audit-log row for the `mcp_tool_call` BEFORE the 500 fired downstream; the guard itself is not implicated.
- **Area:** `apps/agent-query/src/mcp/tools.ts` (MCP tool wiring), `apps/agent-query/src/control-plane/control-plane.client.ts:120` (URL construction), `apps/api/src/products/products.service.ts:143-148` (`getProduct`).

### What's wrong (three layers, compounding)

1. **`get_product` is the *only* product-bound MCP tool that requires `domain_id`** in its inputSchema (`apps/agent-query/src/mcp/tools.ts:78`). The other three (`get_trust_score`, `get_lineage`, `get_slo_summary`) take `product_id` alone. From an agent's POV `product_id` is globally unique — having to *also* pass `domain_id` is API surface inconsistency that's unique to this tool.
2. **The MCP server doesn't enforce `required` from the tool's inputSchema.** A `tools/call` request that omits `domain_id` is accepted; the handler at `tools.ts:82` calls `client.getProduct(orgId, args.domain_id, args.product_id)` with `args.domain_id === undefined`.
3. **`control-plane.client.getProduct` template-strings `undefined` straight into the URL** (`control-plane.client.ts:122` — `/organizations/${orgId}/domains/${domainId}/products/${productId}`). The control plane controller accepts the literal string `"undefined"` as the `:domainId` route param, passes it to `ProductsService.getProduct`, which feeds it to TypeORM's `findOne({ where: { domainId } })`. Postgres rejects: `invalid input syntax for type uuid: "undefined"`. NestJS `ExceptionsHandler` returns 500.

### Reproduction (verified on demo 2026-05-30)

Agent JWT for a seeded agent with a valid grant + ref on a real product:

```
POST /mcp/messages?sessionId=…
{"jsonrpc":"2.0","id":2,"method":"tools/call",
 "params":{"name":"get_product","arguments":{"product_id":"<uuid>"}}}
```

Returns on the SSE stream:

```
data: {"jsonrpc":"2.0","id":2,"result":{
  "content":[{"type":"text","text":"Error: Request failed with status code 500"}],
  "isError":true}}
```
...
```
query failed: ... WHERE (... AND ("DataProductEntity"."domain_id" = $3)) ... PARAMETERS: [..., "undefined"]
error: invalid input syntax for type uuid: "undefined"
[Nest] ERROR [ExceptionsHandler] invalid input syntax for type uuid: "undefined"
```

The same call with `domain_id` included works (the grant + ref enforce; the product details return).

### What "done" looks like (the call this issues to make)

Three candidate fix shapes. They are not mutually exclusive — the right answer is probably to do at least the first two:

- **A — Make `domain_id` optional in `get_product` and look it up from `product_id`.** Aligns this tool's surface with `get_trust_score` / `get_lineage` / `get_slo_summary`. Best for agent DX (`product_id` is the natural identifier). Requires the control-plane to expose either a `domain_id`-less route or an internal lookup. Recommended.
- **B — MCP server enforces `required` from inputSchema** at the `tools/call` boundary; missing required args return a structured MCP error (`isError: true` with `tool_args_missing` reason), not a downstream 500. Defensive belt; should be done independently of A. The `tool-scope-map`-style discipline already exists in this codebase ([CLAUDE.md: "a newly added MCP tool not in `tool-scope-map.ts` denies by default"](../../CLAUDE.md)) — required-arg enforcement is a natural extension.
- **C — Reject literal `"undefined"` (and `"null"`) at the API controller boundary** for any `:uuid` route param. Belt-and-suspenders against the same class of bug elsewhere. Lowest priority.

### Why this matters for the demo arc

PR #226's seed extension makes the agent-side MCP arc work end-to-end *except for this tool*. Marketing Copilot's `get_trust_score` returns real data (verified). Risk Assistant's tight-scope `get_trust_score` denial fires correctly (`CONNECTION_REFERENCE_SCOPE_VIOLATION` audit + notification path verified). But the most natural opening beat in a demo — "*tell me about this product*" — currently 500s unless the demo driver knows to pass `domain_id`. Worth fixing before the next agent-centric demo.

**Related:** [PR #226](https://github.com/provenance-logic/provenance/pull/226) (seed extension that surfaced this), [B-076](./resolved.md#B-076) (prior MCP-path silent regression — same "✅ Complete ≠ continuously verified" class), `documents/demo-scripts/demo-asset-inventory.md` Section 9 (tool×product matrix).
...
- **Status:** **RESOLVED 2026-05-24.** Closed in two halves. (1) *Honesty* — V32 (#177) cut the `ConnectorType` enum / registration UI / CHECK constraint down to the types that actually work, so the platform stopped advertising the 9 unimplemented types; the post-OSR deferral of streaming / REST / dbt / Fivetran / BigQuery / MySQL is recorded in PRD OS3.6 / OS3.7 (anchor decision 5, Option 3+4). (2) *Completeness* — the **Snowflake tranche shipped end-to-end and was live-verified** (PRs #197 sketch, #198 probe+schema, #199 discovery crawl, #200 OBJECT_DEPENDENCIES lineage, #201 in-product integration, #202 consumer-destination snippets + cross-org Secure Data Sharing). The first-class connector set is now **PostgreSQL + S3 + Databricks + Snowflake**, all working inbound; Snowflake additionally works outbound (destination snippets) and cross-org (native shares, verified mounting a real share between two Snowflake accounts). Every connector type the platform advertises now actually works — B-063's core concern is fully closed. **Remaining (NOT blocking, enrichment only):** Snowflake `ACCESS_HISTORY` query-derived lineage (Layer 4b); BigQuery / MySQL as future F3.2a tranches by demand. See [`snowflake-integration-sketch.md`](../architecture/snowflake-integration-sketch.md) and the status board's 2026-05-24 Snowflake section.
