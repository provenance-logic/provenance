...
- **Discovered:** 2026-05-07, during the cumulative fresh-clone simulation for the OSR onboarding-blocker arc (the verification step for PR #70).

**Symptom.** After `docker compose up -d` from a fresh clone, two containers come up with their endpoints fully responsive but Compose marks them `(unhealthy)` indefinitely:

```
$ docker ps --format "table {{.Names}}\t{{.Status}}"
provenance-api      Up 27 seconds (unhealthy)
provenance-minio    Up 27 seconds (unhealthy)
...
```
...
```
$ curl http://localhost:3001/api/v1/health   → HTTP 200
$ curl http://localhost:9000/minio/health/live → HTTP 200
```

Functionally the stack works — the README path completes, seed runs, login succeeds. But:

1. The misleading `(unhealthy)` status alarms anyone reading `docker compose ps` while walking the README.
2. Any future `depends_on: { api: { condition: service_healthy } }` (e.g. an agent-query / kong-routes / smoke-test sidecar) will never start.

**Root cause.** Both healthchecks invoke a tool that is not in the container image:

- `apps/api/Dockerfile`'s development stage uses `node:20-slim` (Debian-slim, minimal). The `infrastructure/docker/docker-compose.yml` `api` healthcheck is `wget -qO- http://localhost:3001/api/v1/health`. Verified: `command -v wget` inside the running container returns nothing. Healthcheck logs show `/bin/sh: 1: wget: not found` on every probe.
- `provenance-minio` runs `quay.io/minio/minio:RELEASE.2024-04-06T05-26-02Z`. The `infrastructure/docker/docker-compose.yml` `minio` healthcheck is `curl -sf http://localhost:9000/minio/health/live`. Verified: only `mc` (the MinIO Client) is present in `/usr/bin`; no `curl`, `wget`, `nc`, or `netcat`. Healthcheck logs show `curl: command not found`.

This is the same class of bug as the now-resolved B-010 (OPA distroless image had no `wget`). Both PR #66 (which fixed the *path* of the api healthcheck — B-021 item 4 — from `/health` to `/api/v1/health`) and the original authors of these healthchecks missed that the *tool* itself is wrong for the chosen base image. The team has not noticed because nothing in the running stack currently `depends_on: { ... condition: service_healthy }` for either container, so the unhealthy state has no functional consequence — it just looks broken.

**Proposed fix.** Two one-line healthcheck changes, no Dockerfile changes:

1. **`api`:** replace the `wget` healthcheck with a Node-based one. Node 20+ ships built-in `fetch`, and `node` is by definition the api container's primary binary. Verified inside the running container: `node -e "fetch('http://localhost:3001/api/v1/health').then(r => process.exit(r.ok ? 0 : 1)).catch(() => process.exit(1))"` exits 0 on healthy, 1 on unhealthy. Patch in `infrastructure/docker/docker-compose.yml` and `infrastructure/docker/docker-compose.dev.yml`:
...
2. **`minio`:** replace the `curl` healthcheck with `mc ready`. The MinIO Client (`mc`) is shipped with the minio image and the standard pattern from MinIO's own ops docs is `mc ready <alias>`. The compose can either configure an alias on first boot or use `mc ready local` if a default alias is present. Patch in `infrastructure/docker/docker-compose.yml`:
   ...
   Verify the `local` alias resolves on first boot in the unmodified minio image — if it does not, an alternative is `["CMD", "mc", "alias", "set", "local", "http://localhost:9000", "${MINIO_ROOT_USER:-provenance}", "${MINIO_ROOT_PASSWORD:-provenance_dev_password}"]` as a one-shot init step before the healthcheck, or use `["CMD-SHELL", "mc alias set local http://localhost:9000 \"${MINIO_ROOT_USER:-provenance}\" \"${MINIO_ROOT_PASSWORD:-provenance_dev_password}\" >/dev/null 2>&1; mc ready local"]` to combine alias setup and readiness check.

Audit other healthchecks in the same compose files for the same class of bug while we're at it (e.g. neo4j, opensearch, kong) — confirm each one's tool is actually shipped in the image. Note: `web` uses `wget` and is `node:20-alpine` (alpine ships busybox `wget`), so it is fine.

**Pattern.** Healthcheck commands must use a tool that is provably present in the container image. Distroless and "-slim" base images strip standard utilities; minio, postgres, redpanda, and some others ship custom-purpose binaries (`mc`, `pg_isready`, `rpk`) that should be preferred over generic curl/wget. Audit checklist when adding any healthcheck:
1. Identify the image's base layer (alpine, debian-slim, distroless, custom).
2. Confirm the chosen tool is present (run the check command interactively in a fresh container before trusting it in compose).
3. Prefer the image's primary binary over generic shell utilities — they will never be missing.

**Related (resolved).** B-010 — OPA 0.63.0 distroless image had no shell or `wget`. Same class of bug; fixed by switching to `["CMD", "/opa", "eval", "true"]`. The same audit pattern applies here.
