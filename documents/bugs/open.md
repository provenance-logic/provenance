...
- **Status:** Open — design pass closed via [ADR-010](../architecture/adr/ADR-010-rls-by-default.md) (#158); **ADR-010 implementation step 2 (the `agents.service.ts` load-bearing fix) landed in #TBD** — every `findOne` / `find` on `agent_identities` and `agent_trust_classifications` now filters on `orgId = ctx.orgId`; the raw audit-log queries in `getOversight` filter on `org_id` too; `listAgents` rejects with 403 if the query `orgId` does not match the caller's JWT. **Remaining ADR-010 steps:** (1) ESLint custom rule for the explicit-filter pattern repo-wide; (3) URL-param convention check (`:orgId` standardization, all-`*Id`-params grep); (5) smoke-test cross-tenant layer when B-060 CI lands. The route move under `/organizations/:orgId/agents/...` (ADR-010's preferred shape for step 2) was deferred — the where-clause filter is even tighter than post-lookup assertion (the DB doesn't return the foreign-org row), so it closes the load-bearing leak without coupling to a frontend / MCP route refactor.
