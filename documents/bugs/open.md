...
- **Update 2026-05-31 (README-reconciliation pass):** corrected an overstatement in this entry's own title and pillar-3 framing that had propagated into the public README ("agent-consumer access is API-only with no write UI"). That is too broad. `apps/web/src/features/agents/AgentsPage.tsx` ships a full **Register an agent** form (`RegisterAgentForm` → `agentsApi.register`, which mints the Keycloak client and reveals the client secret once), and `AgentDetailPage.tsx` surfaces grants, connection references (including the legacy-ref badge), and classification history. The genuinely-missing write path is **narrow**: requesting and approving a *connection reference* (the web consent client `apps/web/src/shared/api/consent.ts` is `list`/`get` only) and trust-classification changes (deferred per the `AgentsPage.tsx` comment). Agent registration and full visibility already have a UI; only the connection-reference request/approve form is absent.
...
The platform's *headline* differentiator ("agents as first-class consumers") is the path with the most unaided-operator friction. Agent **registration** is UI-driven (`AgentsPage.tsx` `RegisterAgentForm`), and `AgentDetailPage.tsx` *displays* references + grants read-only (including the legacy-ref distinction). The remaining gap is the **connection-reference** write path specifically: there is no UI to *request* or *approve* a connection reference (`apps/web/src/shared/api/consent.ts` exposes only `list` / `get`; no request/approve/deny/revoke action anywhere in the web app). ~6 manual steps, ~25–30 min per agent:
- **Register the agent** (UI form available) → mints a Keycloak client + secret, but the **secret is shown exactly once** (lose it = `rotate-secret`); the human oversight contact must already exist in Keycloak or registration 400s (`agents.service.ts:90`).
...
- **Severity:** **Medium** — *downgraded from High on 2026-05-31* (see Update 2026-05-31). PAT auth shipped (#218) and is the accepted, code-aligned path per ADR-012, so the auth *method* is no longer a self-service blocker; the residual is the one-time Snowflake network-policy account step (now surfaced in the in-product credential guidance) plus the strategic OAuth / credential-provenance call, both demand-gated. Matt's end-of-session assessment 2026-05-25 ("I thought we were OSR, but we are not") was correct for the connector layer *at that time*, before PAT landed.
- **Status:** Open (residual is strategic, not a blocker). The *credential-entry* half is fixed (ADR-013 / #220 — paste a secret in the GUI, vaulted at rest); PAT authentication shipped (#218) and the dev Snowflake connector validates green; OAuth (ADR-012 method A) remains the demand-gated north star.
...
**Update 2026-05-31 (README-reconciliation pass):** the auth *method* question is settled and shipped — **PAT authentication landed in #218** (ADR-012 slice 2, live-verified `SELECT 1` → `200`; #219), and `buildSnowflakeAuth()` in `connector-probe.service.ts` resolves a credential to a `pat | keypair` discriminated union (PAT → `PROGRAMMATIC_ACCESS_TOKEN` header, preferred when both are present). ADR-012 records the direction as Accepted: **PAT is phase 1 (shipped); OAuth "Sign in with Snowflake" is phase 3, demand-gated and may be deferred indefinitely** ("PAT-plus-a-good-form is allowed to be the answer"). This means the "expert service-account recipe" described above was a **workaround for one account's network-policy configuration, not a platform requirement** — a PAT minted under an ordinary user authenticates once the account has a network policy, and the network-policy prerequisite is now stated in the in-product credential guidance (`connector-specs.ts`, ADR-012 Decision 2.B). What's genuinely left is the **strategic** OAuth-vs-PAT / credential-provenance call (authority + blast-radius), which is a deliberate future decision, not an open defect. Severity downgraded High → Medium accordingly. The public README's "Known limitations" note was corrected to match.

**Related:** [ADR-013](../architecture/adr/ADR-013-connector-credential-self-service-vault.md) (credential vault, shipped), [ADR-012](../architecture/adr/ADR-012-connector-auth-and-guided-registration.md) (guided form + auth methods; PAT shipped, OAuth demand-gated). The recurring "✅ Complete ≠ a real user can do it" pattern: B-076, B-077, B-079, and the `local-env` dead-end this session.
