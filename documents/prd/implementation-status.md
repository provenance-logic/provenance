...
| F7.46 | Onboarding Experience | Implemented | Inline first-run wizard at the top of `/dashboard` with five steps (confirm org, invite team, register connector, publish product, invite agent) backed by a new `identity.principal_preferences` table + `GET/PATCH /me/preferences`. Steps are skippable individually; the wizard as a whole is dismissible and resumable. Three steps wire to live destinations (org summary, OrgRolesPage, NewProductForm via the first available domain); two steps render skip-only "Coming soon" panels pending B-025 (connector registration UI) and B-026 (agent registration UI). The "Sample data" button from the roadmap is deferred to B-027. Once the user marks all five steps done/skipped the wizard never auto-opens again. |
