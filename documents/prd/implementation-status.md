...
| F2.8a | Output Port Source Object Binding | **Partially implemented — backend done, producer UI pending** | Backend half closed 2026-05-23. Migration V33 added `source_registration_id UUID NULL REFERENCES connectors.source_registrations(id)` and `source_object_path TEXT NULL` to `port_declarations` with an index on the FK. `PortDeclarationEntity`, `Port` type, OpenAPI Port + DeclarePortRequest + UpdatePortRequest schemas all carry the new fields. `ProductsService.declarePort`/`updatePort` accept the binding with a same-org validation guard against cross-org source registrations. `ProductEnrichmentService.resolveColumnSchema` now consults the bound source's latest `schema_snapshot` and maps to `ProductColumnSchema`. `ProductEnrichmentService.resolveFreshness` populates `lastRefreshedAt` from the same snapshot's `capturedAt`. 5 new tests pin the F2.8a behavior. **Remaining:** producer "Pick from discovered objects" UI on port create/edit (frontend work; separate PR). |
...
| F10.17 | Destination Snippet Generation | **Partially implemented (5 of 6 destinations real for sql_jdbc)** | `ConnectionPackageService.generateSnippetForPort` shipped 2026-05-22 (#166); Power BI added 2026-05-23 (Phase 5.10 first piece) — for `sql_jdbc` ports, generates a `.pbids` JSON the consumer saves and double-clicks; protocol picked from host pattern (postgresql / snowflake / databricks-sql / mysql). Six destinations live: Python (all 6 interface types), dbt (sql_jdbc), sql_client + jdbc (sql_jdbc), **power_bi (sql_jdbc)**. Tableau deferred. Frontend tool-picker dropdown in `PortsTab` replaces the static `CONSUMPTION_GUIDANCE` placeholder. Remaining: Tableau snippet (TDS or Web Data Connector); integration with F10.14 catalog-name addressing; integration with F10.15 situation routing; databricks-sql `.pbids` enrichment with SQL-warehouse path (needs a `warehousePath` field on the connection-details contract). |
