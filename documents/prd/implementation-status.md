...
| F7.48 | Provenance Account Surface | **Implemented end-to-end (closes OSR blocker 2026-05-24)** | New requirement landed 2026-05-24 (PRD v1.6); shipped same day. **Backend:** `GET /api/v1/me` returns the profile shape (principalId, keycloakSubject, principalType, displayName, email, org{id,name,slug}, roles[]) — composed from JWT claims + request-context roles + a single OrgEntity primary-key lookup. New `MeModule`, `MeController`, `MeService`. OpenAPI spec at `packages/openapi/me.yaml`. **Frontend:** new `UserMenu` component replaces the prior naked Sign-out button at the sidebar bottom — avatar with initials + display name + org name; click expands a dropdown with "Account" (→ `/account`) and "Sign out". `AccountPage` at `/account` renders the read-only profile (identity card with avatar + name + email + principal type; identity details with org + role pills + click-to-copy principal ID; actions section with Change password + Sign out). Change-password uses `keycloak.login({action: 'UPDATE_PASSWORD', redirectUri})` which deep-links to the Keycloak hosted password-change flow and returns to /account on success. **Deviation from PRD wording:** PRD said "top-right avatar dropdown" — but the app uses a left-sidebar layout (no top bar). Implementation puts the avatar at the sidebar bottom (Linear/Notion/Slack pattern for sidebar layouts); PRD wording amended in the same PR. **Out of scope (still, per PRD F7.48):** in-app edit of display name/email, profile photo, notification preferences UI, multi-org switcher, per-user aggregates. **Test coverage:** 4 service-layer tests + 7 UserMenu tests + 1 OpenAPI spec. |
...
- ~~**F7.48 — Provenance Account Surface.**~~ ✅ Closed 2026-05-24. Same-day implementation: backend `GET /api/v1/me`, frontend `UserMenu` sidebar component + `AccountPage` at `/account` with read-only profile, click-to-copy principal ID, change-password deep-link to Keycloak, and sign-out. See F7.48 row above for the per-piece breakdown.
