...
| F10.19 | Credential Lifespan UX | **Partially implemented — backend done, frontend pending** | Backend half closed 2026-05-24. Migration V35 adds `access_grants.expiry_warning_7d_sent_at` sibling to the existing 14d idempotency marker. New notification categories `access_grant_expiring_7d` (urgent reminder) and `access_grant_renewed` (auto-renew confirmation) with channel defaults + display labels in both renderer maps. `AccessNotificationsTriggerWorker.runGrantExpiry7d` fires the 7d-tier reminder; runs alongside the existing 14d branch on the 5-minute cron. New `AccessService.renewGrant(orgId, grantId, callerPrincipalId)` returns `{mode: 'auto_renewed' \| 'approval_required', grant?, request?}` — per F10.15 situation routing: all output ports declared situation-A-eligible → extend `expiresAt` by the original TTL + reset both warning markers + notify grantee; otherwise → create a new pending access_request linked via `payload.renewalOfGrantId` + notify owner. Cross-org safe per Model A (grant lives in requester's org). 8 new tests pin the worker + service behavior. New endpoint `POST /organizations/:orgId/access/grants/:grantId/renew` (grantee-only at the service layer; no Roles guard since the consumer initiates renewal). **Remaining:** frontend renewal CTA on the notification inbox + the marketplace product detail page (consume the `mode` response and either show "renewed!" or route into the access tab pending approval). |
